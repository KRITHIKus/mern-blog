...
function ThermalCanvas() {
  ...
  useEffect(() => {
    ...
    const heatColor = (t, a) => {
      if (t < 0.33) {
        const g = Math.floor((t / 0.33) * 100)
        return `rgba(0,${g + 80},255,${a})`
      } else if (t < 0.66) {
        const b = Math.floor(((t - 0.33) / 0.33) * 255)
        return `rgba(0,255,${255 - b},${a})`
      } else {
        const r = Math.floor(((t - 0.66) / 0.34) * 255)
        return `rgba(255,${255 - r},0,${a})`
      }
    }
...
export default function SignIn() {
  ...
  // live clock + thermal fluctuation
  useEffect(() => {
  ...
  const handleSubmit = async e => {
    ...
    if (!formData.email || !formData.password) {
      return dispatch(signInFailure('All fields required. Bio-scan failed.'))
    }
    // panel flash
  ...
  const terminalLines = [
    { prompt: '▶', text: 'mask.init()',           cls: '' },
    { prompt: '→', text: 'thermal: active',        cls: 'ok' },
    { prompt: '→', text: 'cloaking: standby',      cls: 'ok' },
    { prompt: '→', text: loading ? 'scanning bio-key...' : 'target: detected', cls: loading ? 'warn' : 'warn' },
    { prompt: '_',  text: '',                       cls: 'cursor' },
  ]

  const btnLabel = () => {
    if (btnState === 'loading') return '⟳ SCANNING BIO-SIGNATURE...'
    if (btnState === 'success') return '✓ BIO-LOCK DISENGAGED'
    ...
    <>
      <style>{`
        ...
        .pred-scan {
          ...
          background: linear-gradient(90deg, transparent, rgba(255,170,0,0.65), rgba(255,51,0,0.4), transparent);
          filter: blur(0.5px);
          animation: predScan 4.5s linear infinite;
          pointer-events: none; z-index: 2;
        }
        @keyframes predScan {
          0%{top:-2px;opacity:0} 5%{opacity:1} 95%{opacity:0.7} 100%{top:100%;opacity:0}
        }
        ...
        .crosshair-wrap {
          ...
          pointer-events: none;
          animation: chDrift 7s ease-in-out infinite;
        }
        @keyframes chDrift {
          0%,100%{transform:translate(-50%,-50%)}
          30%{transform:translate(-53%,-47%)}
          60%{transform:translate(-47%,-53%)}
        }
        .ch {
        ...
        .ch-ring-outer {
          ...
          animation:ringPulse 2.2s ease-in-out infinite 0.4s;
        }
        ...
        .pulse-cell {
          ...
          animation:cellPulse var(--cd,3s) var(--cdelay,0s) ease-in-out infinite;
        }
        @keyframes cellPulse {
          0%,100%{border-color:rgba(255,170,0,0.05);background:transparent}
          50%{border-color:rgba(255,170,0,0.3);background:rgba(255,170,0,0.02);box-shadow:0 0 8px rgba(255,170,0,0.08)}
        }

        /* vignette */
        .auth-vignette {
          position:absolute;inset:0;pointer-events:none;z-index:1;
          background:radial-gradient(ellipse at center,transparent 40%,rgba(5,3,0,0.65) 100%);
        }

        /* HUD data */
        .hud-data {
          position:absolute;font-size:7px;letter-spacing:0.12em;
          color:#ffaa00;font-family:'Orbitron',monospace;z-index:3;
        }
        .hud-data.tr { top:20px;right:40px;text-align:right; }
        .hud-data.bl { bottom:56px;left:20px; }
        .hud-data.br { bottom:56px;right:20px;text-align:right; }
        .hud-val  { color:#ffaa00;opacity:0.9;font-size:8px; }
        .hud-lbl  { opacity:0.4;font-size:6px;letter-spacing:0.14em; }
        .hud-hot  { color:#ff3300 !important; }
        ...
        .auth-pred-logo {
          ...
          letter-spacing:0.22em;color:#ffaa00;
        ...
        .auth-pred-title {
          ...
          line-height:1.2;color:#ffcc66;
          flex:1;display:flex;flex-direction:column;justify-content:center;
        }
        .auth-pred-hl {
          color:#ffaa00;display:block;
          text-shadow:0 0 18px rgba(255,170,0,0.5),0 0 36px rgba(255,170,0,0.2);
        }
        .auth-pred-sub {
          font-size:10px;color:#664400;letter-spacing:0.08em;
        ...
        /* bio terminal */
        .bio-terminal {
          border:1px solid #2a1a00;border-radius:2px;
          background:rgba(5,3,0,0.9);overflow:hidden;
        }
        .bio-term-header {
          background:rgba(42,26,0,0.8);padding:5px 10px;
        ...
        .bio-p  { color:#ffaa00;opacity:0.4; }
        .bio-out { color:#ffcc66; }
        .bio-out.ok   { color:#00ff88; }
        .bio-out.warn { color:#ffaa00; }
        .bio-out.hot  { color:#ff3300; }
        .bio-cursor {
          display:inline-block;width:6px;height:10px;
          background:#ffaa00;opacity:0.8;
        ...
        .auth-status {
          display:flex;align-items:center;gap:10px;
          font-size:7px;color:#664400;letter-spacing:0.1em;
        }
        .status-dot {
        ...
        /* submit overlay flash */
        .auth-flash {
          position:absolute;inset:0;pointer-events:none;opacity:0;z-index:10;
        }
        .auth-flash.active {
          animation:thermalFlash 1s ease forwards;
        }
        @keyframes thermalFlash {
          0%{opacity:0}
          15%{opacity:1;background:rgba(255,170,0,0.05)}
          30%{opacity:0.6;background:rgba(255,51,0,0.03)}
          100%{opacity:0}
        }
        .auth-flash-scan {
          position:absolute;left:0;right:0;height:2px;top:0;opacity:0;
          background:linear-gradient(90deg,transparent,rgba(255,170,0,0.7),rgba(255,51,0,0.5),transparent);
          filter:blur(0.5px);
        }
        .auth-flash.active .auth-flash-scan {
          animation:flashScan 1s ease forwards;
        }
        @keyframes flashScan{
          0%{top:0;opacity:0}8%{opacity:1}92%{opacity:0.4}100%{top:100%;opacity:0}
        }

        /* success overlay */
        .auth-success {
          position:absolute;inset:0;background:rgba(5,3,0,0.97);
          display:flex;flex-direction:column;align-items:center;justify-content:center;
          gap:14px;z-index:20;opacity:0;pointer-events:none;
        ...
        .auth-success-text {
          ...
          letter-spacing:0.2em;color:#00ff88;
        ...
        .auth-eyebrow {
          ...
          color:#664400;margin-bottom:8px;
        ...
        .auth-form-title {
          ...
          letter-spacing:0.2em;color:#ffaa00;margin-bottom:28px;
        ...
        .auth-fld-label {
          ...
          color:#664400;margin-bottom:6px;
        }
        .auth-fld-icon { color:rgba(255,170,0,0.35);font-size:8px; }
        .auth-input-wrap { position:relative; }
        .auth-input {
          width:100%;background:#0a0700;
          border:1px solid #2a1a00;border-radius:2px;
          padding:11px 44px 11px 14px;
          color:#ffcc66;font-family:'JetBrains Mono',monospace;
          font-size:13px;outline:none;letter-spacing:0.06em;
          transition:border-color 0.2s,box-shadow 0.2s,background 0.2s;
          caret-color:#ffaa00;
        }
        .auth-input:focus {
          border-color:#ffaa00;background:rgba(30,15,0,0.9);
          box-shadow:0 0 0 2px rgba(255,170,0,0.06),0 0 14px rgba(255,170,0,0.07);
        }
        ...
        .auth-field-bar {
          ...
          transition:width 0.3s ease;border-radius:0 0 2px 2px;
        }
        ...
        .auth-pwd-toggle {
          ...
          background:none;border:none;border-left:1px solid #2a1a00;
        ...
        .auth-pwd-toggle-lbl {
          font-size:6px;letter-spacing:0.1em;text-transform:uppercase;line-height:1;
        }

        /* submit */
        .auth-submit {
          width:100%;margin-top:6px;
          background:linear-gradient(135deg,rgba(255,170,0,0.92),rgba(255,100,0,0.88));
          color:#050300;
          font-family:'Orbitron',monospace;font-size:10px;font-weight:900;
          letter-spacing:0.16em;text-transform:uppercase;
          padding:13px;border:none;border-radius:2px;cursor:pointer;
          position:relative;overflow:hidden;
          box-shadow:0 0 18px rgba(255,170,0,0.12);
          transition:box-shadow 0.15s,transform 0.1s,background 0.3s;
        }
        .auth-submit:hover {
          box-shadow:0 0 28px rgba(255,170,0,0.35),0 0 50px rgba(255,100,0,0.12);
          transform:translateY(-1px);
        }
        .auth-submit:active { transform:translateY(0); }
        .auth-submit.loading {
          background:linear-gradient(135deg,rgba(255,170,0,0.5),rgba(255,100,0,0.5));
        }
        .auth-submit.success {
          background:linear-gradient(135deg,rgba(0,255,136,0.85),rgba(0,180,80,0.85));
        }
        .auth-btn-shimmer {
        ...
        @keyframes ripple{to{transform:scale(35);opacity:0}}

        /* or */
        .auth-or {
          display:flex;align-items:center;gap:10px;
          font-size:7px;color:#2a1a00;letter-spacing:0.14em;margin:14px 0;
        }
        .auth-or::before,.auth-or::after { content:'';flex:1;height:1px;background:#2a1a00; }

        /* google */
        .auth-google {
          width:100%;background:transparent;border:1px solid #2a1a00;
          border-radius:2px;padding:11px;cursor:pointer;
          display:flex;align-items:center;justify-content:center;gap:9px;
          color:#ffcc66;font-size:11px;font-family:'JetBrains Mono',monospace;
        ...
        /* footer link */
        .auth-foot {
          font-size:9px;color:#664400;margin-top:16px;
          display:flex;align-items:center;gap:6px;
        }
        ...
        /* alert */
        .auth-alert {
          margin-top:12px;padding:9px 12px;border-radius:2px;
        ...
        .shake { animation:authShake 0.4s ease !important; }

        /* ── RESPONSIVE ── */
        @media (max-width:768px) {
          .auth-page { grid-template-columns:1fr; }
          .auth-left  { min-height:240px;padding:28px 24px; }
          .auth-right { padding:28px 24px; }
          .auth-pred-title { font-size:18px; }
          .bio-terminal { display:none; }
          .thermal-legend { display:none; }
        }
      `}</style>
      ...
      <div className="auth-page">
        ...
        <div className="auth-left">
          ...
          {[
            ...
            <div key={i} className="pulse-cell" style={{
              top:c.top, left:c.left, width:c.w, height:c.w,
              '--cd':c.cd, '--cdelay':c.d,
            }}/>
          ))}
          ...
          <div className="hud-data tr">
            ...
            <div className="hud-lbl" style={{marginTop:4}}>TARGET LOCK</div>
            <div className="hud-val hud-hot">ACQUIRED</div>
          </div>
          ...
          <div className="hud-data br">
            <div className="hud-lbl">SIGNAL</div>
            <div className="hud-val hud-ok">SECURE</div>
            <div className="hud-lbl" style={{marginTop:4}}>PROTO</div>
            <div className="hud-val">v2.4.1</div>
          </div>
          ...
          <div className="auth-left-content">
            ...
            <div className="auth-pred-title">
              You are<br/>being<br/>
              <span className="auth-pred-hl">hunted.</span>
              <span className="auth-pred-sub">// Authenticate to survive.</span>
            </div>
            ...
            <div className="thermal-legend">
              {[
                ...
                <div key={label} className="tl-item">
                  <div className="tl-swatch" style={{ background: bg }}/>
                  {label}
                </div>
              ))}
            </div>
            ...
            <div className="bio-terminal">
              <div className="bio-term-header">
                ...
                <div className="bio-term-title">BIO-MASK OS v7.3</div>
              </div>
              <div className="bio-term-body">
                {terminalLines.map((vis, i) => (
                  <div key={i} className={`bio-line ${vis ? 'show' : ''}`}>
                    <span className="bio-p">{terminalLines[i] ? terminalLines[i].prompt || '▶' : ''}</span>
                    {i === 4
                      ? <span className="bio-cursor"/>
                      : <span className={`bio-out ${
                          i === 1 || i === 2 ? 'ok' :
                          i === 3 ? (loading ? 'warn' : 'warn') : ''
                        }`}>
                          {['mask.init()','thermal: active','cloaking: standby',
                            loading ? 'scanning bio-key...' : 'target: detected',''][i]}
                        </span>
                    }
                  </div>
                ))}
        ...
        <div className="auth-right">
          ...
          <div className={`auth-success ${btnState === 'success' ? 'show' : ''}`}>
            ...
            <div className="auth-success-sub">IDENTITY CONFIRMED</div>
          </div>

          <div className="auth-eyebrow">BIO-MASK AUTHENTICATION PROTOCOL</div>
          <div className="auth-form-title">IDENTIFY</div>

          <form onSubmit={handleSubmit}>
            {/* email */}
            <div className="auth-fld">
              <div className="auth-fld-label">
                <span className="auth-fld-icon">◈</span> Neural ID / Email
              </div>
            ...
            <div className="auth-fld">
              <div className="auth-fld-label">
                <span className="auth-fld-icon">◈</span> Bio-Key / Password
              </div>
              <div className="auth-input-wrap">
                <input
                  className="auth-input"
                ...
                >
                  ...
                  <span className="auth-pwd-toggle-lbl">
          ...
          <p className="auth-foot">
            Not registered? <Link to="/signup">ENLIST →</Link>
          </p>
